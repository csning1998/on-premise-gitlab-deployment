...
7. All reads follow the canonical layer chain, in order: `foundation-*`, `shared-*`/`security-*`, `infra-*`, `provision-*`, `platform-*-frontend`. Vault frontend's node exporter reaches the `observability` tenant through a pass-through output on `security-pki`, which already legitimately reads `shared-vault-frontend`, rather than a read from the `platform-*-frontend` tier down to `shared-vault-frontend` that would skip several levels.

## Section 3: Observability Dashboard Platform

`terraform/layers/platform-observability-governance` provisions Grafana's dashboard and datasource configuration.
...
3. It reads `shared-vault-frontend` (via `vault_api_port`) and `security-credentials` for its Vault provider configuration, matching the pattern used by `platform-gitlab-governance` and `platform-harbor-governance`.
