...
### [security-vault-approle] Foundation AppRole & Initial Policy
...
### [security-pki] ACL Source of Truth (SSoT)
...
- **Key Outcomes**:
    ...
    - **Interface**: Exported `management_policies` map for consumption by `provision-vault-oidc`.

### [provision-keycloak-oidc] Identity Provider (Keycloak) Provisioning
...
### [provision-vault-oidc] Vault OIDC Integration & Group Mapping
...
- **Key Outcomes**:
    ...
    - **Dynamic RBAC**:
        - Created **External Identity Groups** based on `security-pki` definitions.
...
sequenceDiagram
    ...
    Identity->>Vault: Attach mapped security-pki Policies
...
1. **Decoupled Policies**: Policies are defined in `security-pki` as raw ACLs, completely unaware of the authentication method (OIDC).
2. **External Group Mapping**: By using `external` groups and aliases in `provision-vault-oidc`, users are assigned permissions based on their Keycloak membership _at the time of login_. No manual policy assignment is needed in Vault.
