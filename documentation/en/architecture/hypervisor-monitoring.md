...
1. A new role, `hypervisor_baseline`, targets the physical host directly through a local Ansible connection (`hosts: localhost`, `connection: local`), invoked from `playbook_hypervisor.yaml`, since the host is not a member of any existing VM inventory and corresponds to no Terraform layer.
