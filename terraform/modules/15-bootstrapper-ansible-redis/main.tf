...
resource "local_file" "inventory" {
  content = templatefile("${path.root}/../../templates/inventory-redis-cluster.yaml.tftpl", {

    # Use the variable passed from the layer
    ansible_ssh_user     = var.vm_credentials.username,
    redis_nodes          = var.redis_nodes,
    redis_allowed_subnet = var.ansible_config.extra_vars.redis_allowed_subnet
  })
  filename = "${var.ansible_config.root_path}/inventory-redis-cluster.yaml"
}
    ...
    */
    command = <<-EOT
      ...
      ansible-playbook \
        ...
        --extra-vars "redis_requirepass=${nonsensitive(var.redis_credentials.requirepass)}" \
        --extra-vars "redis_masterauth=${nonsensitive(var.redis_credentials.masterauth)}" \
        -v \
        ${var.ansible_config.root_path}/playbooks/10-provision-redis.yaml
    EOT
