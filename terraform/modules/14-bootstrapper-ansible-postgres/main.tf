    ...
    */
    command = <<-EOT
      ...
      ansible-playbook \
        ...
        --extra-vars "pg_replication_password=${nonsensitive(var.postgres_credentials.replication_password)}" \
        --extra-vars "pg_superuser_password=${nonsensitive(var.postgres_credentials.superuser_password)}" \
        -v \
