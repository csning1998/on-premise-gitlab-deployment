...
The GitLab Terraform Provider in this layer requires a Personal Access Token (PAT) with administrator privileges to perform API operations. The provider retrieves this token from Vault at the path `secret/on-premise-gitlab-deployment/gitlab/app/pat`.

This token can be provisioned using one of the following methods:
...
If automatic generation is disabled, the token must be generated and stored in Vault manually. Perform the following steps to bootstrap the token:
