...
locals {
  ansible_template_config = {
    ...
    postgres_ha_vip           = module.context.primary_net_config.lb_config.vip
  ...
  ansible_extra_config = {
    pg_replication_password = local.sec_app_credentials.replication_password
    pg_superuser_password   = local.sec_app_credentials.superuser_password
    pg_vrrp_secret          = local.sec_app_credentials.vrrp_secret
