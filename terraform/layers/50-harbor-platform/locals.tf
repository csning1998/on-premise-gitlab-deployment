...
locals {
  dns_hosts = {
    ...
    "${data.terraform_remote_state.postgres.outputs.harbor_postgres_virtual_ip}" = "postgres.iac.local ${data.terraform_remote_state.vault_pki.outputs.pki_configuration.postgres_domains["harbor"][0]}"

    "${data.terraform_remote_state.redis.outputs.harbor_redis_virtual_ip}" = "redis.iac.local ${data.terraform_remote_state.vault_pki.outputs.pki_configuration.redis_domains["harbor"][0]}"

    "${data.terraform_remote_state.minio.outputs.harbor_minio_virtual_ip}" = "s3.iac.local ${data.terraform_remote_state.vault_pki.outputs.pki_configuration.minio_domains["harbor"][0]}"
