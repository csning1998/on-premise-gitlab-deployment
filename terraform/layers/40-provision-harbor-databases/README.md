...
1. **Run an isolated apply for the certificate resource.**
    ...
    terraform apply -auto-approve \
        -target="vault_pki_secret_backend_cert.harbor_db_client" \
        -replace="vault_pki_secret_backend_cert.harbor_db_client"
    ```
...
2. **Verify that the certificate has been updated.**
    ...
    ```bash
    python3 -c 'import json, subprocess; state = json.load(open("terraform.tfstate")); cert = [r["instances"][0]["attributes"]["certificate"] for r in state["resources"] if r["name"] == "harbor_db_client"][0]; subprocess.run(["openssl", "x509", "-noout", "-dates"], input=cert.encode())'
    ```
