...
locals {
  ansible_template_vars = {
    ...
    # Asymmetric Routing (Dynamic Flattened List)
    # Service Segment routes are always added for all runtimes.
    # NAT Segment routes are only added for Kubernetes runtimes, since NAT CIDRs
    # are generated by the K8s overlay (VXLAN) and irrelevant for baremetal/docker.
    vault_static_routes = flatten([
      for s_name, components in local.state.metadata.global_topology_network : [
        ...
        ) if s_name != local.svc_context.s_name || c_name != local.svc_context.c_name
      ]
    ])
  }
