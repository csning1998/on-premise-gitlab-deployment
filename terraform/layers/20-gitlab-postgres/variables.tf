...
variable "postgres_cluster_config" {
  ...
  type = object({
    ...
    ha_virtual_ip   = optional(string, "172.16.140.250")
    inventory_file  = optional(string, "inventory-postgres-gitlab.yaml")
...
variable "postgres_infrastructure" {
  ...
  type = object({
    ...
    postgres_allowed_subnet = optional(string, "172.16.140.0/24")
    storage_pool_name       = optional(string, "iac-gitlab-postgres")
  })
}
