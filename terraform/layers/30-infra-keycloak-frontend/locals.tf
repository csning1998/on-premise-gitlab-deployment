...
locals {
  ...
  topology_cluster = {
    components        = var.keycloak_config
    storage_pool_name = local.storage_pool_name
  }

  # Map dynamic component names back to their single physical identity
  node_identities = {
    for comp_name, comp_config in var.keycloak_config : comp_name => local.svc_identity
  }
}
...
locals {
  ansible_template_vars = {
    ...
    keycloak_fqdn           = local.svc_fqdn
    keycloak_service_domain = local.svc_identity.cluster_name

    # Networking & HA
    keycloak_vip         = local.net_physical_infra.lb_config.vip
    keycloak_port        = local.net_physical_infra.lb_config.ports["https"].frontend_port
    keycloak_node_subnet = local.net_physical_infra.network.hostonly.cidr
    ...
    keycloak_cluster_ips = [
      for comp_name, comp_config in var.keycloak_config : [
