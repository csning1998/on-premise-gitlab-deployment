# infra-gitlab-gitaly-praefect: GitLab Gitaly / Praefect Infrastructure
...
Praefect maintains its own metadata database that maps each repository to its primary and replica Gitaly nodes. A fresh Praefect deployment has an empty database and does not automatically discover repositories already present on the Gitaly nodes' disks.
...
## Praefect HA to Standalone Gitaly Downgrade
...
2.  **Apply `infra-gitlab-gitaly-praefect` to destroy Praefect and Patroni VMs:**

    ```bash
    cd terraform/layers/infra-gitlab-gitaly-praefect
...
3.  **Re-apply `platform-gitlab-frontend` to switch GitLab's endpoint back to Gitaly VIP:**

    ```bash
    cd terraform/layers/platform-gitlab-frontend
    terraform apply -auto-approve
    ```

    `platform-gitlab-frontend` detects `has_praefect = false` (no Praefect nodes in `infra-gitlab-gitaly-praefect` state) and updates the GitLab Helm release to:
