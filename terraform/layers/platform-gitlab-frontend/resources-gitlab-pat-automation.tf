...
# Generates or renews the GitLab root PAT that authenticates the provider in
# platform-gitlab-governance, replacing the manual bootstrap described in that layer's README.
# A new value of gitlab_pat_automation_version forces job recreation. The script in
# templates/gitlab-pat-automation.sh.tftpl remains idempotent while the token is still valid.
