...
- **狀態重新產生**：在保留 `provision-gitlab-databases` 層（Postgres 持久性資料）的同時，對 `platform-gitlab-frontend` 執行 `tofu destroy && tofu apply`，導致 `random_password` 資源被銷毀並重新建立
...
3. 重新套用 GitLab 平台層 `platform-gitlab-frontend`，以使用**新**的機密資訊來建立資料結構描述並植入初始資料。
...
- **狀態持久性**：除非有打算完全清除環境，否則應避免刪除此層的 `terraform.tfstate`
