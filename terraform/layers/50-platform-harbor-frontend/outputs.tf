...
output "ingress_context" {
  ...
  value = {
    load_balancer_ip = local.state.microk8s_provision.harbor_microk8s_virtual_ip
    http_node_port   = local.state.metadata.global_topology_network["harbor"]["frontend"].ports["ingress-http"].backend_port
    https_node_port  = local.state.metadata.global_topology_network["harbor"]["frontend"].ports["ingress-https"].backend_port
  }
}
