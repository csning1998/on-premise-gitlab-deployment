...
module "minio_metrics_token" {
  ...
  vault_auth_mount_path = "kubernetes/gitlab/frontend"
  vault_role_name       = "core-gitlab-frontend-role"
  vault_kv_key          = "${data.terraform_remote_state.vault_pki.outputs.vault_kv_namespace}/gitlab/app/minio_prometheus"
}
