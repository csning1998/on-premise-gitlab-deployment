...
7. **Robot credentials in Vault**. Each robot username and secret is written to Vault at `secret/on-premise-gitlab-deployment/harbor/robots/{team}` so the GitLab layer can consume them. See [60-provision-gitlab/README.md](../60-provision-gitlab/README.md) for how they become CI/CD variables.
